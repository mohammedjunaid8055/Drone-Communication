  ...
  px4_msgs__msg__EstimatorFusionControl__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4_msgs__msg__EstimatorFusionControl * data =
      (px4_msgs__msg__EstimatorFusionControl *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__EstimatorFusionControl__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__EstimatorFusionControl__fini(&data[i]);
        }
        free(data);
        return false;
      }
