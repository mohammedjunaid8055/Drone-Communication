...
class VtePosition(metaclass=Metaclass_VtePosition):
    ...
    def __init__(self, **kwargs):
        ...
        if 'rel_pos' not in kwargs:
            ...
            self.rel_pos = numpy.array(kwargs.get('rel_pos'), dtype=numpy.float32)
            assert self.rel_pos.shape == (3, )
        if 'vel_uav' not in kwargs:
            self.vel_uav = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.vel_uav = numpy.array(kwargs.get('vel_uav'), dtype=numpy.float32)
            assert self.vel_uav.shape == (3, )
        if 'vel_target' not in kwargs:
            self.vel_target = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.vel_target = numpy.array(kwargs.get('vel_target'), dtype=numpy.float32)
            assert self.vel_target.shape == (3, )
        ...
        if 'acc_target' not in kwargs:
            ...
            self.acc_target = numpy.array(kwargs.get('acc_target'), dtype=numpy.float32)
            assert self.acc_target.shape == (3, )
        if 'cov_rel_pos' not in kwargs:
            self.cov_rel_pos = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.cov_rel_pos = numpy.array(kwargs.get('cov_rel_pos'), dtype=numpy.float32)
            assert self.cov_rel_pos.shape == (3, )
        if 'cov_vel_uav' not in kwargs:
            self.cov_vel_uav = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.cov_vel_uav = numpy.array(kwargs.get('cov_vel_uav'), dtype=numpy.float32)
            assert self.cov_vel_uav.shape == (3, )
        if 'cov_bias' not in kwargs:
            self.cov_bias = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.cov_bias = numpy.array(kwargs.get('cov_bias'), dtype=numpy.float32)
            assert self.cov_bias.shape == (3, )
        if 'cov_vel_target' not in kwargs:
            self.cov_vel_target = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.cov_vel_target = numpy.array(kwargs.get('cov_vel_target'), dtype=numpy.float32)
            assert self.cov_vel_target.shape == (3, )
        if 'cov_acc_target' not in kwargs:
            self.cov_acc_target = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.cov_acc_target = numpy.array(kwargs.get('cov_acc_target'), dtype=numpy.float32)
            assert self.cov_acc_target.shape == (3, )
    ...
    def __eq__(self, other):
        ...
        if all(self.rel_pos != other.rel_pos):
            return False
        if all(self.vel_uav != other.vel_uav):
            return False
        if all(self.vel_target != other.vel_target):
            return False
        if all(self.bias != other.bias):
            return False
        if all(self.acc_target != other.acc_target):
            return False
        if all(self.cov_rel_pos != other.cov_rel_pos):
            return False
        if all(self.cov_vel_uav != other.cov_vel_uav):
            return False
        if all(self.cov_bias != other.cov_bias):
            return False
        if all(self.cov_vel_target != other.cov_vel_target):
            return False
        if all(self.cov_acc_target != other.cov_acc_target):
            return False
        return True
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'vel_uav' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'vel_target' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'acc_target' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'cov_rel_pos' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'cov_vel_uav' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'cov_bias' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'cov_vel_target' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'cov_acc_target' field must be a set or sequence with length 3 and each value of type 'float'"
