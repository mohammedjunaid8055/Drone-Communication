...
class ActuatorServos(metaclass=Metaclass_ActuatorServos):
    ...
    def __init__(self, **kwargs):
        ...
        if 'control' not in kwargs:
            self.control = numpy.zeros(15, dtype=numpy.float32)
        else:
            self.control = numpy.array(kwargs.get('control'), dtype=numpy.float32)
            assert self.control.shape == (15, )
    ...
    def control(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 15, \
                "The 'control' numpy.ndarray() must have a size of 15"
            self._control = value
            return
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 True), \
                "The 'control' field must be a set or sequence with length 15 and each value of type 'float'"
