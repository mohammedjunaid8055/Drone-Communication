...
class ActuatorServosTrim(metaclass=Metaclass_ActuatorServosTrim):
    ...
    def __init__(self, **kwargs):
        ...
        if 'trim' not in kwargs:
            self.trim = numpy.zeros(15, dtype=numpy.float32)
        else:
            self.trim = numpy.array(kwargs.get('trim'), dtype=numpy.float32)
            assert self.trim.shape == (15, )
    ...
    def trim(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 15, \
                "The 'trim' numpy.ndarray() must have a size of 15"
            self._trim = value
            return
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 True), \
                "The 'trim' field must be a set or sequence with length 15 and each value of type 'float'"
